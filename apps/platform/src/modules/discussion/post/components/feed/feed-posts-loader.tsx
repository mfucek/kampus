...
import { NoFollowedPostsCard } from './no-followed-posts-card';

const FeedPostsPage: FC<{
	page: PostFeedListItem[];
}> = ({ page }) => {
	return (
		<>
			{page.map((post) => (
...
export const FeedPostsLoader = () => {
	const POST_COUNT = 10;

	const query = api.post.feed.list.useInfiniteQuery(
		{ limit: POST_COUNT },
		{
	...
	return (
		<div className="flex flex-col gap-2">
			{query.data?.pages.map((page, index) => (
				<FeedPostsPage key={index} page={page.posts} />
			))}

			{!hasPosts && <NoFollowedPostsCard />}
